...
impl MiningStrategy for KLeadSelfishMiningStrategy {
    ...
    fn on_mining_block(
        ...
        // 攻撃者がブロックを掘り出したときは常に公開しない（公開ブロック数 = 0）
        // リードL = private_chain_height - public_chain_height
        // 新しいリードL' = L + 1（常に1増える）
        // - L ≤ 0: L' = 1（攻撃開始、秘密チェーン開始）
        // - 1 ≤ L < k: L' = L + 1（リード拡大、kに近づける）
        // - L = k: L' = k + 1（バッファ確保、余裕を作る）
        // - L > k: L' = L + 1（余剰リードをさらに積む、公開はまだ）
