...
stale_rate, honest_only_stale_rate
（いずれも 50% 成功点の attacker_percent で trials 本実行し集約。
 stale_rate は全採掘ブロック、honest_only_stale_rate は honest 採掘のみ。
 集計高さは skip_initial_epochs 以降の評価エポック区間に限定）
...
class StaleRates:
    """--metrics 1 行から読み取った stale 指標のペア。"""
...
def read_stale_rates_from_metrics(metrics_csv: Path) -> StaleRates:
    """シミュレータの --metrics（1 行 CSV）から stale_rate と honest 専用指標を読む。"""
...
def _aggregate_stale_rates(rates: list[StaleRates], stale_stat: str) -> StaleRates:
    ...
    if stale_stat == "median_high":
        ...
        raise ValueError(
            f"stale_stat は mean, median, median_high のいずれか: {stale_stat!r}"
        )
    return StaleRates(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--stale-stat",
        choices=("mean", "median", "median_high"),
        default="median_high",
        help=(
            "stale_rate / honest_only_stale_rate の trials 集約（既定: median_high）。"
            "median_high は偶数試行時に中央2点の上側を採用"
        ),
    )
    return p
...
def main() -> None:
    ...
    for selfish in (True, False):
        ...
        for lambda_delta in lambda_deltas:
            ...
            if prog:
                print(
                    f"  stale_rate / honest_only_stale_rate: running {args.trials} "
