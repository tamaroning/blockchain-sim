...
def plot_attack_success_heatmap_by_block_generation_time():
    ...
    t_gen_grid = np.linspace(t_gen_min, t_gen_max, n_t_gen)
    ...
    boundaries = np.linspace(0.5, 1.0, 11)
    boundaries = np.concatenate([boundaries, [1.001]])
    ...
    cs_fine = ax.contour(X, Y, Z, levels=[0.55, 0.65, 0.75, 0.85], colors="w", linewidths=0.35, alpha=0.5)
    ax.clabel(cs_fine, inline=True, fontsize=8, fmt="%.2f")
    cs75 = ax.contour(X, Y, Z, levels=[0.75], colors=["#f0f0f0"], linewidths=2.0, linestyles="-")
    ax.clabel(cs75, inline=True, fontsize=10, fmt=lambda _lev: "α=0.75")
